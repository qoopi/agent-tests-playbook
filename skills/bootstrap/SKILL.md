...
2. **Install.** Dependencies from the lockfile, `bun install --frozen-lockfile`, `npm ci` or `pnpm install --frozen-lockfile`, then the browsers listed in the `BROWSERS` map of `playwright.config.ts`. Done when both commands exit 0.
